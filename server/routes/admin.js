...
router.post('/login', async(req, res) => {
    try {
        ...
        if (!user) {
            return res.redirect('/login?showAlert=true&danger=Invalid Username');
        }

        const validatePassword = await bcrypt.compare(password, user.password);

        if (!validatePassword) {
            return res.redirect('/login?showAlert=true&danger=Invalid Password');
        }
        ...
        return res.redirect('/dashboard?showAlert=true&success=Login Successfully');
    } catch (error) {
        console.log(error);
        return res.redirect('/login?showAlert=true&danger=Internal Server Error');
    }
});
